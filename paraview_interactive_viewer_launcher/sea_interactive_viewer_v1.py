...
class ActiveView:
    ...
    def __init__(self, input_sl_dll_path, input_etopo_south_nc_path, input_sea_2d_nc_path,
        ...
        # sea 타임스텝 값 기준시 상수 = 1968 년 5월 23 일
        self.DATUM = datetime.datetime(1968, 5, 23, 0, 0, 0)

        # 필요 파일 경로 설정
        self.sl_dll_path = u'%s' % input_sl_dll_path  # StreamLinesRepresentatil.dll 파일 경로
        self.etopo_south_nc_path = u'%s' % input_etopo_south_nc_path  # etopo 육지 및 해양 nc 파일 경로
        self.sea_2d_nc_path = u'%s' % input_sea_2d_nc_path  # sea 모델 nc 파일 경로
        self.time_font_ttf_path = u'%s' % input_time_font_ttf_path  # 표출되는 한글 텍스트 표시에 사용될 ttf 파일 경로
        self.show_time_label = use_time_label  #  뷰 하단에 시간 문자열(YYYY년 MM월 DD일 HH시) 표시할 지 여부 ( Boolean )
        self.view_size_h = int(input_view_size_h)  # 뷰 가로 사이즈
        self.view_size_v = int(input_view_size_v)  # 뷰 세로 사이즈

        # sea 모델 파일명에 포함된 날짜 문자열 가져오기 (sea_YYYYMMDD.nc)
        self.current_time_str = os.path.basename(self.sea_2d_nc_path).split(u'.')[0].split(u'_')[-1]  # 결과 : YYYYMMDD

        #### Load Plug-In "StreamLinesRepresentations"
        sm.LoadPlugin(filename=self.sl_dll_path)  # StreamLinesRepresentation 플러그인 dll(리눅스는 so) 파일 로드

        ############# ETOPO #############
        self.etopo11nc = NetCDFReader(FileName=[self.etopo_south_nc_path])  # etopo nc 파일 읽어오기
        self.etopo11nc.Dimensions = '(lat, lon)'  # etopo nc 파일의 y, x Dimension 설정

        # 읽어온 etopo nc 파일의 표현 방식 설정. 타원체 아님
        self.etopo11nc.SphericalCoordinates = 0

        # 가시화 결과를 표현할 RenderView 생성
        self.renderView1 = GetActiveViewOrCreate('RenderView')

        # RenderView 내에서 XYZ 축 안 보이게
        self.renderView1.OrientationAxesVisibility = 0

        # 입력받은 RenderView 윈도우의 가로 세로 사이즈 설정 (픽셀)
        self.renderView1.ViewSize = [self.view_size_h, self.view_size_v]

        # Interactive 뷰어 내에서 회전 동작 시 자연스럽도록 회전 중심점 재설정 
        self.renderView1.SetPropertyWithName("CenterOfRotation",
                                             [127.13333129882812, 31.52500057220459, -0.38132499158382416])

        # 뷰 윈도우 상단에 로고 이미지 대신 텍스트로 대체
        # 모니터 없이 OffScreen 모드로 렌더링 시, 로고 이미지 표현 불가하여 대체하는 것으로 방침 변경
        self.text1 = Text()  # 텍스트 객체 생성
        self.text1.Text = '국립해양조사원'  # 텍스트 내용 설정
        self.text1Display = Show(self.text1, self.renderView1)  # 텍스트 디스플레이 속성 객체
        self.text1Display.Color = [1.0, 1.0, 1.0]  # 텍스트 컬러 설정 : WHITE
        self.text1Display.WindowLocation = 'UpperLeftCorner'  # 텍스트 표시 위치 : 좌상단
        self.text1Display.FontFamily = 'File'  # 텍스트 폰트 설정 : ttf 파일 사용
        self.text1Display.FontFile = self.time_font_ttf_path  # ttf 파일 경로 설정
        self.text1Display.FontSize = 10  # 텍스트 폰트 사이즈 설정 : 10
        ...
        # ETOPO nc 데이터 표현 옵션 설정 위한 객체 잡기
        ...
        ############# sea 현재 표출 중인 타입스텝 값 스트링(YYYY년 MM월 DD일 HH시) 변환 작업 코드 #############
        if True:
    ...
    def m01_create_avi_by_time_steps(self, input_out_avi_dir, camera_setting="ENTIRE",
        ...
        for time_step in sorted(self.sea_2dnc_TimeStepValues):
            ...
            if not (0 <= time_step_idx <= (len(self.sea_2dnc_TimeStepValues) - 1)):
                pass
            else:
    ...
    def m01_create_avi(self, input_out_avi_dir, camera_setting="ENTIRE",
        ...
        self.extractTimeSteps1.TimeStepRange = [0, len(self.sea_2dnc_TimeStepValues) - 1]
        self.animationScene1.StartTime = sorted(self.sea_2dnc_TimeStepValues)[0]
        self.animationScene1.EndTime = sorted(self.sea_2dnc_TimeStepValues)[-1]
...
if __name__ == '__main__':
    ...
    activeView1 = \
        ActiveView(
            ...
            input_etopo_south_nc_path="D:\\PARAVIEW\\01_sea\\DATA\\etopo11.nc",
            input_sea_2d_nc_path="D:\\PARAVIEW\\01_sea\\DATA\\sea_20170825.nc",
    ...
    activeView1.m00_02_02_set_view_graphics_ocean(
        dim='2D', var='velocity', scale_min=0, scale_max=1.4, opacity=0.5,
        input_ocean_colorbar_font_ttf_path='D:\\PARAVIEW\\01_sea\\DATA\\fonts\\NanumBarunGothic.ttf',
        colormap_json_path='D:\\PARAVIEW\\01_sea\\DATA\\colormaps\\UVVelocity_color.json'
    )
    activeView1.m00_02_03_set_view_graphics_streamline(
        color_setting='UNIFORM', line_width=2, n_of_particles=2500, max_time_to_live=60, step_len=0.05, alpha=0.05,
        opacity=1,
